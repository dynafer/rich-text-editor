...
const FormatUI = (): IFormatUI => {
	...
	const CreateIconButton = (title: string, iconName: string): HTMLElement =>
		Create({
			...
			title,
			type: 'icon-button',
			html: RichEditor.Icons.Get(iconName),
		});
	...
	const CreateSelection = (title: string, labelText: string = ''): IFormatUISelection => {
		...
		DOM.Insert(Selection, Label, RichEditor.Icons.Get('AngleDown'));
	...
	const CreateOption = (html: string, title?: string, bSelected: boolean = false, bAddIcon: boolean = true): HTMLElement => {
		const option = Create({
			...
			title: title ?? html,
			type: 'option-item',
			html: Str.Merge(html, bAddIcon ? RichEditor.Icons.Get('Check') : '')
		});
	...
	const CreateHelper = (title: string): HTMLElement => Create({
		...
		title,
		type: 'helper',
		html: RichEditor.Icons.Get('AngleDown')
	});
	...
	const CreateInputWrapWithOptionList = (editor: Editor, opts: IFormatUIInputWrapOptions): IFormatUIInputWrapWithOptionList => {
		...
		DOM.On(OptionWrapper, ENativeEvents.click, event => RichEditor.PreventEvent(event));
		...
		const cancelButton = Create({
			tagName: 'button',
			title: texts.cancel,
			html: Str.Merge(RichEditor.Icons.Get('Close'), texts.cancel)
		});
		...
		const insertButton = Create({
			tagName: 'button',
			title: insertText,
			html: Str.Merge(RichEditor.Icons.Get('Check'), insertText)
		});
		...
		if (bUpdatable && Type.IsFunction(removeCallback)) {
			...
			const removeButton = Create({
				tagName: 'button',
				title: removeText,
				html: Str.Merge(RichEditor.Icons.Get('Trash'), removeText)
			});
