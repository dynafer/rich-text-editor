...
export const BeforeInner = (doc: Document, selector: TElement, ...insertions: (TElement | string)[]) => {
	...
	Arr.Each(Arr.Reverse(insertions), insertion => {
		...
		selector.appendChild(Type.IsString(insertion) ? doc.createTextNode(insertion) : insertion);
	});
};

export const AfterInner = (doc: Document, selector: TElement, ...insertions: (TElement | string)[]) => {
	if (!NodeType.IsElement(selector) && !NodeType.IsNode(selector)) return;
